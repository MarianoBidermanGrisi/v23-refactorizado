...
CORRECCIÓN: Sin cambios en lógica de trading, solo correcciones de logs
"""
import os
import sys
import signal
...
def main():
    ...
    try:
        ...
        orchestrator.logger.info("🎯 Sistema listo y ejecutándose...")
        orchestrator.logger.info("📊 Para ver estado: curl http://localhost:5000/health")
        orchestrator.logger.info("🛑 Para detener: Ctrl+C")
        ...
        while orchestrator.running:
            try:
                ...
                if not orchestrator.bot_thread or not orchestrator.bot_thread.is_alive():
                    orchestrator.logger.warning("⚠️ Bot thread no está activo, reiniciando...")
                    orchestrator.iniciar_bot_background()
            except KeyboardInterrupt:
                break
            except Exception as e:
                orchestrator.logger.error(f"Error en bucle principal: {e}")
                time.sleep(10)
    except Exception as e:
        orchestrator.logger.error(f"❌ Error fatal en main: {e}")
        sys.exit(1)
    finally:
